...
Integrates an SQLight database, where predefined folders and customer-maintainable documents are stored.
Folders are automatically created via script; the customer manages permissions on them.
Files are manually uploaded by the customer (max. 1000 files per upload, 5 GB per file).
File permissions correspond to those of the respective folders.

#### 1.7.2 SharePoint Integration via Microsoft Entra ID:

The connection to SharePoint is established via an API with authentication using Microsoft Entra ID (formerly Azure Active Directory).
Tenant-ID, Client-ID, and Client Secret are required for setup and are stored in a configuration file.
If these data are not available, the AI Audit Assist app must first be registered by the customer
in Microsoft Entra ID at https://kunde.sharepoint.com/sites/.

##### 1.7.2.1 Accessing SharePoint:

Via the Microsoft Graph API, AI Audit Assist can access and edit SharePoint content after successful OAuth authentication.
...
- **Custom MCP APIs:** for integration with external tools, systems or data sources, e.g., feeds (optional)
