
==== License Management

This page allows you to manage the license required to operate the {application} application. Licenses can be updated if an existing license has been replaced.

You can also contact {application} support by email using the integrated button. Clicking it opens your default email client with the recipient field pre-filled.

image::../images/Abbildung-30.jpg[Administration - License Management, title="Administration - License Management", width=400]

A completely new license file can be uploaded after selecting it.

image::../images/Abbildung-31.jpg[Administration - License Management - Upload License, title="Administration - License Management - Upload License", width=400]

