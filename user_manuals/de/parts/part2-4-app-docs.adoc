...
über den Refresh Button das RAG aktualisiert oder einzelne Dokumente gelöscht werden. Da es bei sehr vielen Dokumenten zu Zeitverzögerungen kommen kann,
 wurde ein Lazy-Loading implementiert. Dadurch können über das Reload Icon die einzelnen Embeddings (Informationen (meist Wörter, Sätze, Bilder oder andere Daten) in mathematische Vektoren), manuel nachgeladen werden. 
