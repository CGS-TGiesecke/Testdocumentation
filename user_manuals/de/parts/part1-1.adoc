...
Hier können einzelne Anfragen gestartet werden. Alle Chatverläufe werden nur im Browser Storage gespeichert und sind nach dem Browser schließen 
...
Es kann vor dem Ausführen von Anfragen ausgewählt werden, ob der Dokumentenort(RAG) *oder* die Inhalte derDatenbank zur Wissensfindung durchsucht werden sollen. 
...
Die Berechtigungen auf Ordner und enthaltene Dokumente werden bei der Suche über RAG berücksichtig.
Nutzt man das RAG werden bei der Antwort die Top 8 Embedding für die Antwort und ein Link zur jeweiligen Quelle angezeigt. 
