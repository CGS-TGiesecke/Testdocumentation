=== Tabellarische Gegenüberstellung ({application}, MS Copilot, ChatGPT (Standalone))
...
|Unterstützung des gesamten Revisionszyklus |Ja (Risikobewertung bis Maßnahmen-verfolgung) |Nur indirekt über Nutzer-Prompts |Nur indirekt über Nutzer-Prompts
|===
endif::[]
...
|Herstellerunab-hängigkeit |Hoch – freie Wahl und Kombination verschiedener Provider|Niedrig – stark an Microsoft-Stack gebunden |Niedrig – an OpenAI-Ökosystem gebunden
...
|Eignung für hochsensible Daten |Sehr hoch (on-premise, rollen-basiertes RAG) |Eingeschränkt, je nach Compliance-Vorgaben |Eingeschränkt, meist nicht für streng regulierte Bereiche
|===
...
|Formularbasierte Use Cases 

|Ja, fertige Formulare für Kunden
ifeval::[{arc-assist} == 1]
Audit-
endif::[]
Use-Cases
...
Use-Case-Ergebnissen in Chat

|Ja, nahtlos als Kontext weiter nutzbar |Nur manuell durch Copy/Paste|Nur manuell durch Copy/Paste
|===
...
|Integriertes RAG |Ja, speziell für Kundendokumente|Teilweise (über M365/SharePoint-Integration) |Nur über separate, technisch aufwändige Integrationen
|DMS-ähnliche Dokumenten-übersicht |Ja (Filter nach Typ, Quelle, Inhalt, Tags)|Nicht in dieser Form |Nein
...
|Löschen einzelner Embeddings|Ja („Ausgewählte Embeddings löschen“)|Nein|Nein
...
|Ja, aber generisch (M365 Admin Center) | Eingeschränkt (v.a. Organisations-einstellungen)

|Nutzer- & Rollenmanagement 

|Ja, mit 
ifeval::[{arc-assist} == 1]
 revisionsspezifischer
endif::[]
ifeval::[{cgs-assist} == 1]
 kundenspezifischer 
endif::[]
...
|Ja, aber nicht auf RAG/Kunde zugeschnitten | Beschränkt (v.a. Organisation vs. Nutzer)

|Rollenbasierter Zugriff auf RAG-Ordner

| Ja (für alle 
ifeval::[{arc-assist} == 1]
 kodierten Audit-Use-Cases)
endif::[]
ifeval::[{cgs-assist} == 1]
 kodierten Standard-Use-Cases) 
endif::[]

|Nein (Prompts dezentral bei Nutzern) | Nein (Prompts beim einzelnen User)
...
|Lizenzverwaltung | Ja (via Lizenz-File) | Ja (MS-Lizenzmodell, aber generisch) | Ja (Account-/Plan-basiert, nicht Kundenspezifisch)
|===
...
|Zentral durch {author} 
ifeval::[{arc-assist} == 1]
 und in Kooperation mit Internen Revisionen
endif::[]

|Durch Kunde selbst |Durch Kunde selbst
...
|Ja, zentral und für alle Kunden|Nur wenn der Kunde intern optimiert |Nur wenn der Kunde intern optimiert
...
|Skaleneffekte| Sehr hoch – Weiterentwick-lungen kommen allen Kunden zugute |Gering – jeder Kunde optimiert für sich |Gering – jeder Kunde optimiert für sich
|===
...
|Umgang mit Ausfällen einzelner LLMs |Fallback auf alternative LLMs durch Mehr-Provider-Setup|Abhängig von Microsoft / Azure |Abhängig von OpenAI.
|Reduktion geopolitischer Risiken |Ja, durch Nutzung europäischer/selbst gehosteter Modelle möglich|Eingeschränkt, US-Unternehmen|Eingeschränkt, US-Unternehmen
|Abhängigkeit von einem LLM Anbieter |Gering|Hoch |Hoch
|===