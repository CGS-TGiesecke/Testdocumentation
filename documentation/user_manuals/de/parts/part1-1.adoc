...
Hier werden, bei vorhandener Berechtigung, alle automatisierten bereitgestellten Externleseverfahren gelistet und verwaltet.
Die markierte Nummer (rot) zeigt die Anzahl der aktuellen Ergebnisse das letzten Laufs aller Rubriken. Wird die Lesebestätigung von einem User in einer Rubrik gesetzt, 
wird die Anzeige der Ergebnisse(rot) neu berechnet.
