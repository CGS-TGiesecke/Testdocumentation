...
#### 1.7.1 AI Audit Assist DMS:

Bindet eine SQLight-Datenbank an, in der festgelegte Ordner und kundenpflegeleichte Dokumente gespeichert werden.
Ordner werden automatisiert per Skript angelegt; Rechte darauf verwaltet der Kunde selbst.
Dateien werden manuell vom Kunden hochgeladen (max. 1000 Dateien pro Upload, 5 GB pro Datei).
Dateiberechtigungen entsprechen denen der jeweiligen Ordner.

#### 1.7.2 SharePoint-Anbindung via Microsoft Entra ID:

Die Anbindung zu SharePoint erfolgt über eine API mit Authentifizierung per Microsoft Entra ID (früher Azure Active Directory).
Für die Einrichtung sind Tenant-ID, Client-ID und Client Secret nötig, die in einer Konfigurationsdatei hinterlegt werden.
Sind diese Daten nicht vorhanden, muss die Applikation AI Audit Assist vom Kunden erst in 
Microsoft Entra ID unter https://kunde.sharepoint.com/sites/ registriert werden.

##### 1.7.2.1 Zugriff auf SharePoint:

Über die Microsoft Graph API kann AI Audit Assist nach erfolgreicher OAuth-Authentifizierung auf SharePoint-Inhalte zugreifen und diese bearbeiten.
