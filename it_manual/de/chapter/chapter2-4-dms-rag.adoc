...
* **Client-ID:** Die Client-ID der SharePoint-Anwendung mit den erforderlichen Berechtigungen (Client-Secrets) für den Datenzugriff.
...
Registrierung einer Anwendung in Microsoft Entra ID ist der erste Schritt, wenn eine Applikation die Microsoft Identitätsdienste (z. B. Anmeldung mit Microsoft-Konto, Zugriff auf APIs) verwendet.
...
** Melden Sie sich mit Ihrem Konto und Administratorberechtigungen beim Azure-Portal an. Dort wird die **Tenant-ID** angezeigt.
...
und ein Geheimnis (**Client-Secret**: (z.B. Sites.Read.All oder Sites.Selected)) oder Zertifikat. 
