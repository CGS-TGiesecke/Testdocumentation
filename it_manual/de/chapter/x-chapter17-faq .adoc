...
|Wer ist für den laufenden Betrieb verantwortlich?|Das hängt vom gewählten Betriebsmodell ab. Bei SaaS/Managed Service liegt der technische Betrieb überwiegend bei {author}, beim On-Prem-/Kundenbetrieb ist der Kunde für Infrastruktur, OS, Docker/Kubernetes verantwortlich; {author} unterstützt bei Applikationsupdates und -konfiguration. Details siehe Verantwortlichkeitsübersicht in den "RACI-Matrix".
...
|Wie wird die Lösung bereitgestellt?|Über ein Docker Skript auf einem Linux-Server.
|Ist eine spezielle Azure-Integration notwendig?|Nein. Nur API-Key und Endpoint werden benötigt.
|Ist eine VM zwingend erforderlich?|Nein. Jeder Linux-Server mit Docker ist geeignet.
.2+|Benötigt die Lösung eine dauerhafte Internetverbindung? 
| Cloud-Provider (Azure oder AWS): Ja 
...
.4+|Welchen LLM-Provider soll ich wählen?
...
|Wie setze ich Azure OpenAI auf?| Siehe "Unterstützte LLM-Provider (Bereitstellung durch den Kunden)"
|Wie setze ich AWS Bedrock auf?| Siehe "Unterstützte LLM-Provider (Bereitstellung durch den Kunden)"
...
|Müssen wir für den LLM bezahlen?|Ja. Der Kunde muss selbst den LLM Provider bereitstellen (siehe "Kostenübersicht LLM-Provider").
.3+|Wie erfolgt sie Authentifizierung?
|über die lokale Benutzer-/Passwortverwaltung (DB des Kunden)
|über SAML-Authentifizierung mit Microsoft Entra ID
|über OpenID Connect Authentifizierung mit Microsoft Entra ID
...
|Benutzer kann sich nicht anmelden.|Stellen Sie sicher, dass der Benutzer im AD aktiv ist und die zugewiesene Rolle in der Benutzerverwaltung der Applikation korrekt hinterlegt wurde.
Prüfen sie die korrekte Konfiguration der Authentifizierung (reidirect-_url) evtl Schreibfehler.
.3+|Welche externen Dokumente/Verzeichnisse können integriert werden?
|{application} unterstützt:
|SharePoint Integration über MS Entra ID
|E-Mail Integration über MS Entra ID
|===