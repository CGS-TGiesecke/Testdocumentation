...
This is an SQLight DB integration in {application}, where system-defined folders and customer-maintained documents are stored.
Folders are created in the database via script according to customer requirements, and are initially visible to everyone.
The required permissions for roles on the folders are set by the customer.

Files are uploaded manually by the customer.
The supported number of files per upload is 1000, and the maximum file size is typically 5 GB (Windows default).
Permissions for individual files correspond to those of the directories.
...
* **Client-ID:** The client ID of the SharePoint application with the required permissions (ClientSecrets) for data access.
...
This is the first step when an application uses Microsoft identity services (e.g., Microsoft account login, API access).
