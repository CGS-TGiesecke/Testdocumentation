...
|Who is responsible for ongoing operations?|This depends on the chosen operating model. With SaaS/Managed Service, technical operation is mainly handled by {author}; for on-premises/customer operation, the customer is responsible for infrastructure, OS, Docker/Kubernetes; {author} supports with application updates and configuration. See responsibility overview in the IT requirements for details.
